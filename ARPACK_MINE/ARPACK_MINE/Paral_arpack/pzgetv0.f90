...
!
    ...
    SUBROUTINE pzgetv0(comm, ido ,bmat, itry, initv, n, j, v, ldv, resid, rnorm, &
        & ipntr, workd, workl, ierr)
    USE DEBUG_MODULE
    USE STAT_MODULE
    IMPLICIT NONE
    ...
    COMPLEX( kind = 8 ), INTENT(INOUT)::WORKL(2*J)
    ...
    INTEGER( kind = 4 )::idist, iseed(4), iter, msglvl, jj, myid, igen
    REAL( kind = 8 ):: rnorm0
    COMPLEX( kind = 8):: cnorm, cnorm2, cnorm_buf
...
!
   ...
   30 continue
      ...
      call zgemv('C', n, j-1, one, v, ldv, workd, 1, zero, workl(j+1), 1)
      call MPI_ALLREDUCE( workl(j+1), workl, j-1, MPI_DOUBLE_COMPLEX , MPI_SUM, comm, ierr)
...
!
      !if (msglvl .gt. 2) then
      !    call pdvout(comm, logfil, 1, rnorm0, ndigit, '_getv0: re-orthonalization ; rnorm0 is')
      !    call pdvout(comm, logfil, 1, rnorm, ndigit, '_getv0: re-orthonalization ; rnorm is')
      !end if
...
!
   ...
   50 continue
      ...
      !   call pzvout  (comm, logfil, 1, cnorm2, ndigit, '_getv0: B-norm of initial / restarted starting vector')
      !end if
      if (msglvl .gt. 2) then
         call pzvout  (comm, logfil, n, resid, ndigit, '_getv0: initial / restarted starting vector')
      end if
      ido = 99
