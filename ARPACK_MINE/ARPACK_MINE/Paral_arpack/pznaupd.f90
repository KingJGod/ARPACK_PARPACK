...
!-----------------------------------------------------------------------
!
    SUBROUTINE PZNAUPD(comm, ido, bmat, n, which, nev, tol, resid, ncv, v, ldv, iparam, &
        &     ipntr, workd, workl, lworkl, rwork, info )
    ...
    !     %------------------------%
    !
    INTEGER( kind = 4 )::bounds, ierr, ih, iq, ishift, iupd, iw
    INTEGER( kind = 4 )::ldh, ldq, levec, mode, msglvl, mxiter, nb
    INTEGER( kind = 4 )::nev0, next, np, ritz, j

    SAVE  bounds, ih, iq, ishift, iupd, iw
    SAVE  ldh, ldq, levec, mode, msglvl, mxiter, nb
    SAVE  nev0, next, np, ritz
    !
    !     %----------------------%
...
!     %-------------------------------------------------------%
!
      call pznaup2( comm, ido, bmat, n, which, nev0, np, tol, resid, mode, iupd, &
...
!
      ...
      if (msglvl .gt. 0) then
         !call pivout(comm, logfil, 1, mxiter, ndigit,'_naupd: Number of update iterations taken')
         !call pivout(comm, logfil, 1, np, ndigit,'_naupd: Number of wanted "converged" Ritz values')
         call pzvout(comm, logfil, np, workl(ritz), ndigit, '_naupd: The final Ritz values')
         call pzvout(comm, logfil, np, workl(bounds), ndigit, '_naupd: Associated Ritz estimates')
      end if
...
!
 ...
 1000    format (//,                                                    & 
     ...
     &      5x, '= Version Number: ', ' XYC  1.0' , 16x, ' =',/         &
     &      5x, '= Version Date:   ', ' 6/20/2018' , 14x,   ' =',/      &
     &      5x, '=============================================',/       &
