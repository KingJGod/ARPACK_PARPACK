...
!
    ...
    EXTERNAL zdscal, pzmout, pzvout, arscnd
    !
    !     %--------------------%
    !     | External functions |
    !     %--------------------%
...
! 
      if (msglvl .gt. 2) then
          call pzmout (comm, logfil, n, n, h, ldh, ndigit, '_neigh: Entering upper Hessenberg matrix H ')
      end if
! 
!     %----------------------------------------------------------%
...
!
      ...
      if (msglvl .gt. 1) then
         call pzvout (comm, logfil, n, bounds, ndigit, '_neigh: last row of the Schur matrix for H')
      end if
!
!     %----------------------------------------------------------%
...
!
      if (msglvl .gt. 1) then
         ...
         call pzvout (comm, logfil, n, workl, ndigit, '_neigh: Last row of the eigenvector matrix for H')
      end if
!
!     %----------------------------%
...
!
      if (msglvl .gt. 2) then
         call pzvout (comm, logfil, n, ritz, ndigit,'_neigh: The eigenvalues of H')
         call pzvout (comm, logfil, n, bounds, ndigit,'_neigh: Ritz estimates for the eigenvalues of H')
      end if
!
      call arscnd(t1)
      tceigh = tceigh + (t1 - t0)
