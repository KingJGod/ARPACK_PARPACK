...
!
!-----------------------------------------------------------------------
SUBROUTINE PZNEUPD(comm , rvec  , howmny, select, d    ,&
...
!
  INTEGER( kind = 4 ),INTENT(INOUT)::COMM
    ...
    INTEGER( kind = 4 )::bounds, ierr  , ih    , ihbds, iheig , nconv
    INTEGER( kind = 4 )::invsub, iuptri, iwev  , j    , ldh   , ldq
    INTEGER( kind = 4 )::mode  , msglvl, ritz  , wr   , k     , irz
    INTEGER( kind = 4 )::ibd   , outncv, iq    , np   , numcnv, jj
    INTEGER( kind = 4 )::ishift, nconv2
    COMPLEX( kind = 8 )::rnorm, temp, vl(1)
    REAL( kind = 8 )::conds, sep, rtemp, eps23
    LOGICAL::reord
    !
    !     %----------------------%
    ...
    EXTERNAL zunm2r, ztrmm, pzvout, pivout, zlahqr
    !
    !     %--------------------%
    !     | External Functions |
    !     %--------------------%
    !
    REAL( kind = 8 )::dznrm2, pdlamch10, dlapy2
...
!
      ...
      if (msglvl .gt. 2) then
         call pzvout(comm, logfil, ncv, workl(irz), ndigit,'_neupd: Ritz values passed in from _NAUPD.')
         call pzvout(comm, logfil, ncv, workl(ibd), ndigit,'_neupd: Ritz estimates passed in from _NAUPD.')
      end if
!     
      if (rvec) then
!
...
!
         np     = ncv - nev
         ishift = 0
         call pzngets(comm, ishift, which, nev, np, workl(irz),workl(bounds))

         if (msglvl .gt. 2) then
            call pzvout(comm,logfil, ncv, workl(irz), ndigit,'_neupd: Ritz values after calling _NGETS.')
            call pzvout(comm,logfil, ncv, workl(bounds), ndigit,'_neupd: Ritz value indices after calling _NGETS.')
         end if
!
!        %-----------------------------------------------------%
...
!
         ...
         do 11 j = 1,ncv
            ...
            if(numcnv .lt. nconv .and. dlapy2( dble (workl(ibd+jj-1)),dimag(workl(ibd+jj-1))).le. tol*rtemp) then
               select(jj) = .true.
               numcnv = numcnv + 1
...
!
         !if (msglvl .gt. 2) then
         !    call pivout(comm, logfil, 1, numcnv, ndigit,'_neupd: Number of specified eigenvalues')
         !    call pivout(comm, logfil, 1, nconv, ndigit,'_neupd: Number of "converged" eigenvalues')
         !end if 
...
!
     ...
     &                ierr        )
         ...
         if (msglvl .gt. 1) then
            call pzvout(comm, logfil, ncv, workl(iheig), ndigit,'_neupd: Eigenvalues of H')
            call pzvout(comm, logfil, ncv, workl(ihbds), ndigit,'_neupd: Last row of the Schur vector matrix')
            if (msglvl .gt. 3) then
               call pzmout(comm, logfil, ncv, ncv, workl(iuptri), ldh, ndigit,'_neupd: The upper triangular matrix ')
            end if
         end if
         if (reord) then
!
...
!
     ...
     &                  workev, ncv, ierr)
            ...
            if (msglvl .gt. 2) then
                call pzvout (comm, logfil, ncv, workl(iheig), ndigit,'_neupd: Eigenvalues of H--reordered')
                if (msglvl .gt. 3) then
                   call pzmout (comm, logfil, ncv, ncv, workl(iuptri), ldq, ndigit,'_neupd: Triangular matrix after re-ordering')
                end if
            end if
         end if
!
...
!        %----------------------------------------------------------%
!
         call zgeqr2(ncv, nconv, workl(invsub),ldq, workev, workev(ncv+1),ierr)
!
!        %--------------------------------------------------------%
...
!
            if (msglvl .gt. 2) then
               ...
               call pzvout(comm, logfil, nconv, workl(ihbds), ndigit,'_neupd: Last row of the eigenvector matrix for T')
               if (msglvl .gt. 3) then
                  call pzmout(comm, logfil, nconv, ncv, workl(invsub), ldq, ndigit, '_neupd: The eigenvector matrix for T')
               end if
            end if
!
!           %---------------------------------------%
...
!     %-----------------------------------------------------------%
!    
      if (types .eq. 'SHIFTI') then
         do 60 k=1, nconv
            d(k) = one / workl(iheig+k-1) + sigma
  60     continue
      end if

      if (types .ne. 'REGULR' .and. msglvl .gt. 1) then
         call pzvout (comm, logfil, nconv, d, ndigit,'_neupd: Untransformed Ritz values.')
         call pzvout (comm, logfil, nconv, workl(ihbds), ndigit,'_neupd: Ritz estimates of the untransformed Ritz values.')
      else if ( msglvl .gt. 1) then
         call pzvout (comm, logfil, nconv, d, ndigit,'_neupd: Converged Ritz values.')
         call pzvout (comm, logfil, nconv, workl(ihbds), ndigit,'_neupd: Associated Ritz estimates.')
      end if
!
!     %-------------------------------------------------%
...
!     %-------------------------------------------------%
!
      if (rvec .and. howmny .eq. 'A' .and. types .eq. 'SHIFTI') then
!
!        %------------------------------------------------%
