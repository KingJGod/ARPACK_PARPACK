...
!-----------------------------------------------------------------------
!
    SUBROUTINE pznapps(comm, n, kev, np, shift, v, ldv, h, ldh, resid, q, ldq, &
        &     workl, workd )
    USE PCONTEXT_MODULE
    USE DEBUG_MODULE
    USE STAT_MODULE
    ...
    EXTERNAL   pzvout, zlaset, dlabad, pzmout, arscnd, pivout
    !
    !     %--------------------%
    !     | External Functions |
    !     %--------------------%
    !
    REAL( kind = 8 )::zlanhs, pdlamch10, dlapy2
...
!         if (msglvl .gt. 2 ) then
!            call pivout (comm, logfil, 1, jj, ndigit, '_napps: shift number.')
!            call pzvout (comm, logfil, 1, sigma, ndigit, '_napps: Value of the shift ')
!         end if
!
         istart = 1
   20    continue
...
!
            ...
            if ( abs(dble(h(i+1,i))) .le. max(ulp*tst1, smlnum) )  then
               !if (msglvl .gt. 0) then
               !   call pivout (comm, logfil, 1, i, ndigit, '_napps: matrix splitting at row/column no.')
               !   call pivout (comm, logfil, 1, jj, ndigit, '_napps: matrix splitting with shift number.')
               !   call pzvout (comm, logfil, 1, h(i+1,i), ndigit, '_napps: off diagonal element.')
               !end if
               iend = i
               h(i+1,i) = zero
               go to 40
   ...
   40    continue
         ...
         !if (msglvl .gt. 2) then
         !    call pivout (comm, logfil, 1, istart, ndigit, '_napps: Start of current block ')
         !    call pivout (comm, logfil, 1, iend, ndigit, '_napps: End of current block ')
         !end if
!
!        %------------------------------------------------%
...
!
      ...
      if (msglvl .gt. 1) then
         !call pzvout (comm, logfil, 1, q(kplusp,kev), ndigit,'_napps: sigmak = (e_{kev+p}^T*Q)*e_{kev}')
         !call pzvout (comm, logfil, 1, h(kev+1,kev), ndigit,'_napps: betak = e_{kev+1}^T*H*e_{kev}')
         !call pivout (comm, logfil, 1, kev, ndigit, '_napps: Order of the final Hessenberg matrix ')
         if (msglvl .gt. 2) then
            call pzmout (comm, logfil, kev, kev, h, ldh, ndigit,'_napps: updated Hessenberg matrix H for next iteration')
         end if
