...
!-----------------------------------------------------------------------
!
    SUBROUTINE pznaup2(comm, ido, bmat, n, which, nev, np, tol, resid, mode, iupd, &
    ...
    EXTERNAL zsortc , zswap , pzmout , pzvout , pivout, arscnd
    !
    !     %--------------------%
    !     | External functions |
    !     %--------------------%
    ...
    REAL( kind = 8 )::pdznorm2 , pdlamch10 , dlapy2
...
!
   10 continue
      ...
      if (getv0) then
         call pzgetv0 (comm, ido, bmat, 1, initv, n, 1, v, ldv, resid, rnorm, ipntr, workd, workl, info)
...
!     %----------------------------------------------------------%
!
      call pznaitr (comm, ido, bmat, n, 0, nev, mode, resid, rnorm, v, ldv, h, ldh, ipntr, workd, workl, info)
...
! 
 1000 continue
         ...
         !if (msglvl .gt. 0) then
         !   call pivout (comm, logfil, 1, iter, ndigit, '_naup2: **** Start of major iteration number ****')
         !end if
! 
!        %-----------------------------------------------------------%
...
!
         ...
         !if (msglvl .gt. 1) then
         !   call pivout (comm, logfil, 1, nev, ndigit, '_naup2: The length of the current Arnoldi factorization')
         !   call pivout (comm, logfil, 1, np, ndigit, '_naup2: Extend the Arnoldi factorization by')
         !end if
!
!        %-----------------------------------------------------------%
!        | Compute NP additional steps of the Arnoldi factorization. |
...
!
   ...
   20    continue
         ...
         call pznaitr (comm, ido, bmat, n, nev, np, mode, resid, rnorm, v, ldv, h, ldh, ipntr, workd, workl, info)
         ...
         !if (msglvl .gt. 1) then
         !   call pdvout (comm, logfil, 1, rnorm, ndigit, '_naup2: Corresponding B-norm of the residual')
         !end if
! 
!        %--------------------------------------------------------%
...
!        %--------------------------------------------------------%
!
         call pzneigh ( comm, rnorm, kplusp, h, ldh, ritz, bounds, q, ldq, workl, rwork,  ierr)
...
!
   ...
   25    continue
         ...
         if (msglvl .gt. 2) then
            ...
            kp(2) = np
            kp(3) = nconv
            call pivout (comm, logfil, 3, kp, ndigit, '_naup2: NEV, NP, NCONV are')
            call pzvout (comm, logfil, kplusp, ritz, ndigit,'_naup2: The eigenvalues of H')
            call pzvout (comm, logfil, kplusp, bounds, ndigit,'_naup2: Ritz estimates of the current NCV Ritz values')
         end if
!
!        %---------------------------------------------------------%
...
!
 ...
 30      continue
         ...
         if ( (nconv .ge. nev0) .or. (iter .gt. mxiter) .or. (np .eq. 0) ) then
            ...
            if (msglvl .gt. 4) then
               call pzvout(comm, logfil, kplusp, workl(kplusp**2+1), ndigit,'_naup2: Eigenvalues computed by _neigh:')
               call pzvout(comm, logfil, kplusp, workl(kplusp**2+kplusp+1), ndigit,'_naup2: Ritz eistmates computed by _neigh:')
            end if
!     
!           %------------------------------------------------%
...
!
            ...
            if (msglvl .gt. 1) then
               call pzvout (comm, logfil, kplusp, ritz, ndigit,'_naup2: Sorted eigenvalues')
               call pzvout (comm, logfil, kplusp, bounds, ndigit,'_naup2: Sorted ritz estimates.')
            end if
!
!           %------------------------------------%
...
!
         ...
         if (msglvl .gt. 0) then
            !call pivout (comm, logfil, 1, nconv, ndigit,'_naup2: no. of "converged" Ritz values at this iter.')
            if (msglvl .gt. 1) then
               kp(1) = nev
               kp(2) = np
               call pivout (comm, logfil, 2, kp, ndigit, '_naup2: NEV and NP are')
               call pzvout (comm, logfil, nev, ritz(np+1), ndigit,'_naup2: "wanted" Ritz values ')
               call pzvout (comm, logfil, nev, bounds(np+1), ndigit,'_naup2: Ritz estimates of the "wanted" values ')
            end if
         end if
...
!
         ...
         if (msglvl .gt. 2) then 
            !call pivout (comm, logfil, 1, np, ndigit, '_naup2: The number of shifts to apply ')
            call pzvout (comm, logfil, np, ritz, ndigit, '_naup2: values of the shifts')
            if ( ishift .eq. 1 ) then
               call pzvout (comm, logfil, np, bounds, ndigit,'_naup2: Ritz estimates of the shifts')
            endif
         end if
!
!        %---------------------------------------------------------%
...
!        %---------------------------------------------------------%
!
         call pznapps(comm, n, nev, np, ritz, v, ldv, h, ldh, resid, q, ldq, workl, workd)
!
!        %---------------------------------------------%
...
!
         ...
         if (bmat .eq. 'G') then
            ...
            call MPI_ALLREDUCE( cmpnorm_buf, cmpnorm, 1, MPI_DOUBLE_COMPLEX, MPI_SUM, comm, ierr )
         ...
         if (msglvl .gt. 2) then
            !call pdvout (comm, logfil, 1, rnorm, ndigit, '_naup2: B-norm of residual for compressed factorization')
            call pzmout (comm, logfil, nev, nev, h, ldh, ndigit, '_naup2: Compressed upper Hessenberg matrix H')
         end if
