    ...
    EXTERNAL zunm2r, ztrmm, zvout, ivout, zlahqr
    !
    !     %--------------------%
    !     | External Functions |
    !     %--------------------%
    ...
    if (msglvl .gt. 2) then
        ...
        call zvout(logfil, ncv, workl(ibd), ndigit,'_neupd: Ritz estimates passed in from _NAUPD.')
    end if
    !
    if (rvec) then
        !
...
10      continue
        ...
        np     = ncv - nev
        ishift = 0
        call zngets(ishift, which, nev, np, workl(irz), workl(bounds))
        !
        if (msglvl .gt. 2) then
            call zvout (logfil, ncv, workl(irz), ndigit,'_neupd: Ritz values after calling _NGETS.')
            call zvout (logfil, ncv, workl(bounds), ndigit,'_neupd: Ritz value indices after calling _NGETS.')
        end if
        !
        !        %-----------------------------------------------------%
...
11      continue
        ...
        call zlahqr(.true., .true. , ncv, 1, ncv , workl(iuptri),ldh, workl(iheig) , 1,ncv, workl(invsub), ldq,ierr)
        ...
        if (msglvl .gt. 1) then
            ...
            call zvout (logfil, ncv, workl(ihbds), ndigit,'_neupd: Last row of the Schur vector matrix')
            if (msglvl .gt. 3) then
                call zmout (logfil, ncv, ncv, workl(iuptri), ldh, ndigit,'_neupd: The upper triangular matrix ')
            end if
        end if
        !
        if (reord) then
            ...
            !
            if (msglvl .gt. 2) then
                call zvout (logfil, ncv, workl(iheig), ndigit,'_neupd: Eigenvalues of H--reordered')
                if (msglvl .gt. 3) then
                    call zmout(logfil, ncv, ncv,workl(iuptri), ldq, ndigit,'_neupd: Triangular matrix after re-ordering')
                end if
            end if
            !
        end if
...
40          continue
            ...
            if (msglvl .gt. 2) then
                ...
                call zvout (logfil, nconv, workl(ihbds), ndigit, '_neupd: Last row of the eigenvector matrix for T')
                if (msglvl .gt. 3) then
                    call zmout(logfil, ncv, ncv, workl(invsub), ldq, ndigit,'_neupd: The eigenvector matrix for T')
                end if
            end if
            !
            !           %---------------------------------------%
...
50      continue
    ...
    !     |    For TYPES = 'SHIFTI' the transformation is              |
...
60      continue
    ...
    if (types .ne. 'REGULR' .and. msglvl .gt. 1) then
        ...
        call zvout (logfil, nconv, workl(ihbds), ndigit,'_neupd: Ritz estimates of the untransformed Ritz values.')
