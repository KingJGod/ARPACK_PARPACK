    ...
    EXTERNAL   dlabad, zvout, zmout, ivout, arscnd
    !
    !     %--------------------%
    !     | External Functions |
    !     %--------------------%
    ...
    REAL( kind = 8 )::dlamch,  dznrm2, zlanhs, dlapy2
...
30  continue
    ...
    !           %--------------------------------------%
    !
    call zgetv0 (ido, bmat, itry, .false., n, j, v, ldv, resid, rnorm, ipntr, workd, ierr)
...
60  continue
    ...
    !        %------------------------------------------%
    !
    call zgemv ('C', n, j, one, v, ldv, workd(ipj), 1,zero, h(1,j), 1)
    !
    !        %--------------------------------------%
...
80  continue
    ...
    !        %----------------------------------------------------%
    !
    call zgemv ('C', n, j, one, v, ldv, workd(ipj), 1, zero, workd(irj), 1)
    !
    !        %---------------------------------------------%
    ...
    !        %---------------------------------------------%
    !
    call zgemv ('N', n, j, -one, v, ldv, workd(irj), 1, one, resid, 1)
...
90  continue
    ...
    !    call ivout (logfil, 1, j, ndigit, '_naitr: Iterative refinement for Arnoldi residual')
        if (msglvl .gt. 2) then
            rtemp(1) = rnorm
            rtemp(2) = rnorm1
            call dvout (logfil, 2, rtemp, ndigit,'_naitr: iterative refinement ; rnorm and rnorm1 are')
        end if
    end if
    !
    !        %-----------------------------------------%
...
110     continue
        !
        if (msglvl .gt. 2) then
            call zmout (logfil, k+np, k+np, h, ldh, ndigit, '_naitr: Final upper Hessenberg matrix H of order K+NP')
        end if
        !
        go to 9000
    end if
